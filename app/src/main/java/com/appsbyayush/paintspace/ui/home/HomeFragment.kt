...
//                }
    ...
    private fun setupSyncObserver(processId: String? = null) {
        context?.let {
            Log.d(TAG, "setupSyncObserver: Called")
            val savedSyncProcessId = viewModel.getCurrentSyncProcessId()

            Log.d(TAG, "setupSyncObserver: Saved Sync ID - $savedSyncProcessId")
            Log.d(TAG, "setupSyncObserver: Received Sync ID - $processId")

            if(savedSyncProcessId.isEmpty() && processId.isNullOrEmpty()) {
                binding.llSyncingDrawings.visibility = View.GONE
                Log.d(TAG, "setupSyncObserver: No Sync Process Found -- Returning..")
                return
            }
            ...
            WorkManager.getInstance(it)
                ...
                .observe(viewLifecycleOwner) { workInfo ->
                    Log.d(TAG, "setupSyncObserver Observing.. $from: $workInfo")
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.drawingsFlow.collect { response ->
                    Log.d(TAG, "setupDrawingsCollector: Called $response")
...
//                        rvDrawings.isVisible = response is Resource.Success && !response.data.isNullOrEmpty()
                    ...
                    if(response is Resource.Success && response.data != null) {
                        Log.d(TAG, "setupDrawingsCollector: Submitting List")
...
//                            binding.rvDrawings.scrollToPosition(0)
                        ...
                        is HomeViewModel.Event.DrawingCopiedToDevice -> {
                            Log.d(TAG, "onDrawingCopyToDeviceBtnClick: Inside Event")
    ...
    private fun viewImage(uri: Uri) {
        Log.d(TAG, "onDrawingCopyToDeviceBtnClick: ViewImage Called $uri")
        try {
            ...
            Log.d(TAG, "onDrawingCopyToDeviceBtnClick viewImage: $e")
