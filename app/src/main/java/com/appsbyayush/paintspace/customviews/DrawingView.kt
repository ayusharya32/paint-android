...
class DrawingView(
    ...
    private fun drawPathOnCanvas(canvas: Canvas, drawingItem: DrawingItem, drawToSave: Boolean) {
        Log.d(TAG, "drawPathOnCanvas: Called SAVE $drawToSave")
        drawingItem.drawingPath?.let { currentDrawingPath ->
            ...
            Log.d(TAG, "drawPathOnCanvas: Drawing Path Empty ${currentDrawingPath.path.isEmpty}")
            Log.d(TAG, "drawPathOnCanvas: Current Drawing Path ${currentDrawingPath.strokeWidth}  ${currentDrawingPath.strokeColor}  ${currentDrawingPath.brushType}")
            Log.d(TAG, "drawPathOnCanvas: Drawing Path JSON ${Gson().toJson(currentDrawingPath.path)}")
    ...
    private fun drawImageOnCanvas(canvas: Canvas, drawingItem: DrawingItem, drawToSave: Boolean) {
        Log.d(TAG, "drawImageOnCanvas: Called SAVE $drawToSave")
        drawingItem.drawingImage?.let { currentDrawingImage ->
            ...
            Log.d(TAG, "initiateSelectedItemDragging: Rect: Left - ${currentDrawingImage.rectF.left} " +
                    "Top - ${currentDrawingImage.rectF.top} " +
                    "Right - ${currentDrawingImage.rectF.right} " +
                    "Bottom - ${currentDrawingImage.rectF.bottom}")
        }
    }

    private fun drawTextOnCanvas(canvas: Canvas, drawingItem: DrawingItem, drawToSave: Boolean) {
        Log.d(TAG, "drawTextOnCanvas: Called SAVE $drawToSave")
        drawingItem.drawingText?.let { currentDrawingText ->
            ...
            Log.d(TAG, "initiateSelectedItemDragging: Rect: Left - ${currentDrawingText.rectF.left} " +
                    "Top - ${currentDrawingText.rectF.top} " +
                    "Right - ${currentDrawingText.rectF.right} " +
                    "Bottom - ${currentDrawingText.rectF.bottom}")
        }
    }

    private fun onMotionEventActionDown(motionEvent: MotionEvent) {
        Log.d(TAG, "onMotionEventActionDown: Called X=${motionEvent.x} Y=${motionEvent.y}")
    ...
    private fun changeSelectedItemSize(scaleFactor: Float) {
        _currentSelectedItemFlow.value?.let { selectedItem ->
            ...
            rect?.let { itemRect ->
                ...
                Log.d(TAG, "changeSelectedItemSize: originalImageWidth: $originalImageWidth")
                Log.d(TAG, "changeSelectedItemSize: originalImageHeight: $originalImageHeight")

                Log.d(TAG, "changeSelectedItemSize: currentWidth: ${itemRect.width()}")
                Log.d(TAG, "changeSelectedItemSize: currentHeight: ${itemRect.height()}")

                Log.d(TAG, "changeSelectedItemSize: newImageWidth: $newImageWidth ($scaleFactor)")
                Log.d(TAG, "changeSelectedItemSize: newImageHeight: $newImageHeight ($scaleFactor)")
                ...
                if(newImageWidth <= itemRect.width() && newImageHeight <= itemRect.height()) {
                    Log.d(TAG, "changeSelectedItemSize: Size DEC")
                    ...
                    Log.d(TAG, "changeSelectedItemSize: Size INC")
                    itemRect.left = itemRect.left - widthDifference / 2
                    itemRect.top = itemRect.top - heightDifference / 2
                    itemRect.right = itemRect.right + widthDifference / 2
                    itemRect.bottom = itemRect.bottom + heightDifference / 2
                ...
                Log.d(TAG, "changeSelectedItemSize: Updated Width: ${itemRect.width()}")
                Log.d(TAG, "changeSelectedItemSize: Updated Height: ${itemRect.height()}")
    ...
    private fun calculateRectSidesDistancesFromTouchStart(touchStartX: Float, touchStartY: Float) {
        _currentSelectedItemFlow.value?.let { selectedItem ->
            ...
            itemRect?.let { rect ->
                ...
                Log.d(TAG, "Rect Distances distanceOfSelectedItemRectLeftFromTouchStart: $distanceOfSelectedItemRectLeftFromTouchStart")
                Log.d(TAG, "Rect Distances distanceOfSelectedItemRectTopFromTouchStart: $distanceOfSelectedItemRectTopFromTouchStart")
                Log.d(TAG, "Rect Distances distanceOfSelectedItemRectRightFromTouchStart: $distanceOfSelectedItemRectRightFromTouchStart")
                Log.d(TAG, "Rect Distances distanceOfSelectedItemRectBottomFromTouchStart: $distanceOfSelectedItemRectBottomFromTouchStart")
            }
        }
    }
    ...
    private fun getCurrentDrawingPath(): DrawingPath {
        Log.d(TAG, "getCurrentDrawingPath: Path Color: ${currentStrokeColor}")
        return DrawingPath(
...
//            xfermode = PorterDuffXfermode(PorterDuff.Mode.CLEAR)
    ...
    private fun getImageDrawingPaint(drawingItem: DrawingItem): Paint {
        Log.d(TAG, "getImageDrawingPaint: alpha = ${255 - drawingItem.transparency}")
    ...
    fun setBrushColor(color: Int) {
        ...
        Log.d(TAG, "setBrushColor: Updated Color: ${currentStrokeColor}")
    }
    ...
    fun redoLastOperation() {
        ...
        deletedBrushDrawingItemsList.removeLast()
    ...
    fun bringSelectedItemForward(bringToFront: Boolean = false) {
        _currentSelectedItemFlow.value?.let { currentSelectedItem ->
            Log.d(TAG, "bringSelectedItemForward: Before - $drawingItemsList")
            if(isSelectedItemAtLastPosition()) {
                Log.d(TAG, "bringSelectedItemForward: Item At Last")
                return
            }
            ...
            if(itemsAboveSelectedItem.isEmpty()) {
                Log.d(TAG, "bringSelectedItemForward: No items above")
                return
            }
            ...
            val itemToBringSelectedItemAboveOf = try {
                ...
                Log.d(TAG, "bringSelectedItemForward: $e")
                null
            } ?: return
            ...
            Log.d(TAG, "bringSelectedItemForward: After - $drawingItemsList")
    ...
    fun moveSelectedItemBackward(sendToBack: Boolean = false) {
        _currentSelectedItemFlow.value?.let { currentSelectedItem ->
            Log.d(TAG, "moveSelectedItemBackward: Before - $drawingItemsList")
            if(isSelectedItemAtFirstPosition()) {
                Log.d(TAG, "moveSelectedItemBackward: Item At First")
                return
            }

            val indexOfSelectedItem = drawingItemsList.indexOf(currentSelectedItem)

            Log.d(TAG, "moveSelectedItemBackward: SelectedItem Index Before - $indexOfSelectedItem")

            val itemsBelowSelectedItem = drawingItemsList.subList(0, indexOfSelectedItem)
            Log.d(TAG, "moveSelectedItemBackward: itemsBelowSelectedItem - $itemsBelowSelectedItem")

            if(itemsBelowSelectedItem.isEmpty()) {
                Log.d(TAG, "moveSelectedItemBackward: No items below")
                return
            }
            ...
            val itemToBringSelectedItemAboveOf = try {
                ...
                Log.d(TAG, "moveSelectedItemBackward: $e")
                null
            } ?: return

            Log.d(TAG, "moveSelectedItemBackward: itemToBringSelectedItemAboveOf Before - " +
                    "${drawingItemsList.indexOf(itemToBringSelectedItemAboveOf)}")

            drawingItemsList.remove(currentSelectedItem)

            Log.d(TAG, "moveSelectedItemBackward: itemToBringSelectedItemAboveOf Index After - " +
                    "${drawingItemsList.indexOf(itemToBringSelectedItemAboveOf)}")

            val newIndexPosition = drawingItemsList.indexOf(itemToBringSelectedItemAboveOf)

            drawingItemsList.add(newIndexPosition, currentSelectedItem)
            Log.d(TAG, "moveSelectedItemBackward: After - $drawingItemsList")
    ...
    fun setDrawingItems(drawingItems: List<DrawingItem>) {
        ...
        Log.d(TAG, "setDrawingItems: ${drawingItemsList.map { it.type }}")
    ...
    fun saveDrawingImage(): Uri? {
        ...
        Log.d(TAG, "saveDrawingImage: $drawingItemsList")
        for(drawingItem in drawingItemsList) {
